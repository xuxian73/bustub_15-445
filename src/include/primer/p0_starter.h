 ...
 public:
  ...
  T GetElement(int i, int j) const override {
    if (i < 0 || i >= this->rows_ || j < 0 || j >= this->cols_) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "RowMatrix::GetElement");
    }
    return data_[i][j];
  }
  ...
  void FillFrom(const std::vector<T> &source) override {
    if (source.size() != size_t(this->rows_) * this->cols_) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "RowMatrix::FillFrom() source size incorrect.");
    }
...
class RowMatrixOperations {
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    result = std::make_unique<RowMatrix<T>>(matrixA->GetRowCount(), matrixA->GetColumnCount());
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    result = std::make_unique<RowMatrix<T>>(matrixA->GetRowCount(), matrixB->GetColumnCount());
    for (int i = 0; i < matrixA->GetRowCount(); ++i) {
  ...
  static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB,
    ...
    result = std::make_unique<RowMatrix<T>>(matrixA->GetRowCount(), matrixC->GetColumnCount());
    for (int i = 0; i < matrixA->GetRowCount(); ++i) {
      for (int j = 0; j < matrixC->GetColumnCount(); ++j) {
        ...
        result->SetElement(i, j, sum + matrixC->GetElement(i, j));
      }
    }
    return result;
  }
